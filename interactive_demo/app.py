...
class InteractiveDemoApp(ttk.Frame):
    def __init__(self, master, args, model):
        ...
        #新增
        self.show_heatmap_button = FocusButton(self.control_frame, text='Show Heatmap', command=self._show_heatmap)
        self.show_heatmap_button.pack(side=tk.TOP, fill=tk.X, padx=10, pady=3)
    ...
    def _add_canvas(self):
        self.canvas_frame = FocusLabelFrame(self, text="Image"+self.current_image_path)
    ...
    def _load_image_callback(self):
        ...
        if self._check_entry(self):
            ...
            if len(filename) > 0:
                ...
                self.canvas_frame.config(text=f"Image: \n{filename}")
    ...
    def _save_mask_callback(self):
        ...
        if self._check_entry(self):
            ...
            if self.current_image_path:
                ...
                default_save_name = f"{file_name_without_ext}-1click.png"
            else:
                default_save_name = 'mask.png'

            # filename = filedialog.asksaveasfilename(parent=self.master, initialfile='mask.png', filetypes=[
            filename = filedialog.asksaveasfilename(parent=self.master, initialfile=default_save_name, filetypes=[
                ("PNG image", "*.png"),
                ("BMP image", "*.bmp"),
                ("All files", "*.*"),
    ...
    def _update_image(self, reset_canvas=False):
        ...
        # #新增边界线
        # vis = self.controller.get_visualization(alpha_blend=self.state['alpha_blend'].get(), click_radius=self.state['click_radius'].get())
        # self.canvas.set_image(vis)
    ...
    def _show_heatmap(self):
        heatmap = self.controller.get_heatmap_visualization()

...
MODEL_NAME = 'plainvit_large448_scis_mmt_fmg'
...
def init_model(cfg):
    ...
    phrase_encoder_params = dict(
        enocder_name="ViT-B/32",
        embedding_dim=512,
        out_dim=1024,
    )
    ...
    model = PhraseCLIPscisModel_lora(
        use_disks=True,
        norm_radius=5,
        with_prev_mask=True,
        backbone_params=backbone_params,
        phrase_encoder_params=phrase_encoder_params,
...
def train(model, cfg, model_cfg):
    ...
    trainset = PascalPartMMTFMGDataset(
        cfg.PASCALPART_PATH,
        split='train',
    ...
    valset = PascalPartMMTFMGDataset(
        cfg.PASCALPART_PATH,
        split='val',
        partmask_path=cfg.part_path,
    ...
    trainer = ISTrainer(model, cfg, model_cfg, loss_cfg,
                        ...
                        checkpoint_interval=[(0, 20), (50, 1), (55, 20), (105, 1)],
                        image_dump_interval=300,
                        metrics=[AdaptiveIoU()],
                        max_interactive_points=model_cfg.num_max_points,
                        max_num_next_clicks=3)
    trainer.run(num_epochs=110, validation=False)

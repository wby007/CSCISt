...
class COCO2014Dataset(ISDataset):
    def __init__(self, dataset_path, split='train', **kwargs):
        super(COCO2014Dataset, self).__init__(**kwargs)
        assert split in {'train', 'val', 'test'}

        self.dataset_path = Path(dataset_path)
        self.dataset_split = split
        self._images_path = self.dataset_path / f'{split}2014'
        self._anno_path = self.dataset_path / 'annotations' / f'instances_{split}2014.json'

...
class DualBranchMergedLinear(nn.Linear, lora.LoRALayer):
    ...
    def merge_AB(self):
        ...
        (self.lora_A, self.lora_B) = (self.lora_A, self.lora_B) if (not self.lora_branch) else (
        self.lora_A2, self.lora_B2)
        delta_w = F.conv1d(
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = DualBranchMergedLinear(dim, dim * 3, r=8, enable_lora=[True, False, True], bias=qkv_bias)
...
class VisionTransformer_duallora(nn.Module):
